...
def calculate_demographic_data(print_data=True):
    ...
    #list count of countries with >50K in dataframe
    x = df[df['salary'] == '>50K']['native-country'].value_counts().to_frame()
    #add column with total country count
    x["country-counts"] = df["native-country"].value_counts()
    #add column with percentages
    x["percentages"] = round (100 * (x["native-country"] / x["country-counts"]), 1)
    #return highest percentage
    highest_earning_country = x["percentages"].idxmax()

    highest_earning_country_percentage = x["percentages"].max()
